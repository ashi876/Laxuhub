...
# 设置窗口的初始大小和位置
x_pos = screen_width - 360
root.geometry("360x120+{}+0".format(x_pos))  # 将窗口放在右上角
...
def adjust_window_size():
    ...
    root.geometry("{}x{}+{}+20".format(window_width, window_height, x_pos))
...
def create_menu(event):
    ...
    menu.add_command(label="放大字体", command=increase_font_size)
    menu.add_command(label="缩小字体", command=decrease_font_size)
    menu.add_command(label="关闭右键菜单", command=lambda: close_menu(menu))
    menu.add_command(label="关闭时钟", command=root.quit)
